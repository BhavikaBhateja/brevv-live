/**
 * CTA Particle Field
 * Confetti-like shapes settle into a "W" wave along the bottom of the
 * CTA card. They drift continuously (so they clearly read as "alive",
 * not a static image), and the first time the section scrolls into view
 * a phantom cursor sweeps across the field once — demonstrating the
 * repel interaction so users know to move their cursor. The real cursor
 * repels them and they spring back. Reduced-motion users get a static
 * field, and the physics loop only runs while the section is on screen.
 */
...
export function initCtaParticles() {
